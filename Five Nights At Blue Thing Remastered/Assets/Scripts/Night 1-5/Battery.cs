...
public class Battery : MonoBehaviour {
	...
	void Start () {
        batterytext.text = WhichBatteryString[whichbattery] + " " + batterys[whichbattery] + " %"; //On scene start sets battery to battery 1
    }
	
	// Update is called once per frame
	void Update () {
        size.transform.localScale = new Vector3(batterys[whichbattery] / 20.0f, 0.4f, 0.4f);
        if (catcher && officefade.config &&  100 > textmanager.powerstatus && Input.GetMouseButton(0))
        //if the mouse pointer is over <<siphon>> text, the maintenence panel is up, the main battery is less than 100 and left mouse button has ben clicked then trigger condition
        {
